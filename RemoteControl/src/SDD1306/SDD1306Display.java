...
public class SDD1306Display 
{
     ...
     */
    public SDD1306Display(int width, int height, GpioController gpio, I2CBus i2c, int address, Pin rstPin) throws IOException {
        this.width = width;
        this.height = height;
        this.pages = (height / 8);
     ...
     * @see com.pi4j.io.i2c.I2CFactory#getInstance(int) I2C bus factory
     */
    public SDD1306Display(int width, int height, GpioController gpio, I2CBus i2c, int address) throws IOException {
        this(width, height, gpio, i2c, address, null);
    }


    private void initDisplay() throws IOException {
        if (this.width == SDD1306Constants.LCD_WIDTH_128 && this.height == SDD1306Constants.LCD_HEIGHT_64) {
            this.init(0x3F, 0x12, 0x80);
        } else if (this.width == SDD1306Constants.LCD_WIDTH_128 && this.height == SDD1306Constants.LCD_HEIGHT_32) {
            this.init(0x1F, 0x02, 0x80);
        } else if (this.width == SDD1306Constants.LCD_WIDTH_96 && this.height == SDD1306Constants.LCD_HEIGHT_16) {
    ...
    private void init(int multiplex, int compins, int ratio) {
        this.command(SDD1306Constants.SSD1306_DISPLAYOFF);
        this.command(SDD1306Constants.SSD1306_SETDISPLAYCLOCKDIV);
        this.command((short) ratio);
        this.command(SDD1306Constants.SSD1306_SETMULTIPLEX);
        this.command((short) multiplex);
        this.command(SDD1306Constants.SSD1306_SETDISPLAYOFFSET);
        this.command((short) 0x0);
        this.command(SDD1306Constants.SSD1306_SETSTARTLINE);
        this.command(SDD1306Constants.SSD1306_CHARGEPUMP);

        if (this.vccState == SDD1306Constants.SSD1306_EXTERNALVCC)
            this.command((short) 0x10);
        else
            this.command((short) 0x14);

        this.command(SDD1306Constants.SSD1306_MEMORYMODE);
        this.command((short) 0x00);
        this.command((short) (SDD1306Constants.SSD1306_SEGREMAP | 0x1));
        this.command(SDD1306Constants.SSD1306_COMSCANDEC);
        this.command(SDD1306Constants.SSD1306_SETCOMPINS);
        this.command((short) compins);
        this.command(SDD1306Constants.SSD1306_SETCONTRAST);
        ...
        this.command(SDD1306Constants.SSD1306_SETPRECHARGE);
        ...
        this.command(SDD1306Constants.SSD1306_SETVCOMDETECT);
        this.command((short) 0x40);
        this.command(SDD1306Constants.SSD1306_DISPLAYALLON_RESUME);
        this.command(SDD1306Constants.SSD1306_NORMALDISPLAY);
    }
     ...
     * @see SDD1306Constants#SSD1306_SWITCHCAPVCC
     */
    public void begin() throws IOException {
        this.begin(SDD1306Constants.SSD1306_SWITCHCAPVCC);
    }
     ...
     * @see SDD1306Constants#SSD1306_SWITCHCAPVCC
     * @see SDD1306Constants#SSD1306_EXTERNALVCC
     */
    public void begin(int vccState) throws IOException {
        this.vccState = vccState;
        this.reset();
        this.initDisplay();
        this.command(SDD1306Constants.SSD1306_DISPLAYON);
        this.clear();
        this.display();
    }
    ...
    public synchronized void display() {
        this.command(SDD1306Constants.SSD1306_COLUMNADDR);
        this.command(0);
        this.command(this.width - 1);
        this.command(SDD1306Constants.SSD1306_PAGEADDR);
        this.command(0);
    ...
    public void invertDisplay(boolean invert) {
        if (invert) {
            this.command(SDD1306Constants.SSD1306_INVERTDISPLAY);
        } else {
            this.command(SDD1306Constants.SSD1306_NORMALDISPLAY);
        }
    }
    ...
    public void scrollHorizontally(boolean left, int start, int end) {
        this.command(left ? SDD1306Constants.SSD1306_LEFT_HORIZONTAL_SCROLL : SDD1306Constants.SSD1306_RIGHT_HORIZONTAL_SCROLL);
        this.command(0);
        this.command(start);
        this.command(0);
        this.command(end);
        this.command(1);
        this.command(0xFF);
        this.command(SDD1306Constants.SSD1306_ACTIVATE_SCROLL);
    }
    ...
    public void scrollDiagonally(boolean left, int start, int end) {
        this.command(SDD1306Constants.SSD1306_SET_VERTICAL_SCROLL_AREA);
        this.command(0);
        this.command(this.height);
        this.command(left ? SDD1306Constants.SSD1306_VERTICAL_AND_LEFT_HORIZONTAL_SCROLL :
                SDD1306Constants.SSD1306_VERTICAL_AND_RIGHT_HORIZONTAL_SCROLL);
        this.command(0);
        this.command(start);
        this.command(0);
        this.command(end);
    ...
    public void stopScroll() {
        this.command(SDD1306Constants.SSD1306_DEACTIVATE_SCROLL);
    }
